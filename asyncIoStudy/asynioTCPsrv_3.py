...
# task distribute/route to proper task handler though message broker(router/bus)
# now , use simple message for testing
@asyncio.coroutine
def distribute_tasks(task):
    connection = pika.AsyncioConnection()
    channel = connection.channel()
    channel.basic_publish(exchange='rmstest',
                          routing_key = 'rmsmessage',
                          body = 'Test Message')
    connection.close()
